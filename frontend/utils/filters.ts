...
export const filterProperties = (
  ...
  return properties.filter((property) => {
    ...
    // Temperature range
    if (weather.temperature < filters.tempRange[0] || 
        weather.temperature > filters.tempRange[1]) {
      return false;
    }

    // Humidity range
    if (weather.humidity < filters.humidityRange[0] || 
        weather.humidity > filters.humidityRange[1]) {
      return false;
    }

    // Weather condition
    if (filters.condition !== null && weather.weathercode !== filters.condition) {
      return false;
    }
...
export const getFilteredCount = (properties: Property[]): number => properties.length;
