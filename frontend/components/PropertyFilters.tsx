              ...
              <div className="text-right">
                <p className="text-sm font-medium text-blue-800">
                  {[
                    ...
                    filters.tempRange[0] !== TEMPERATURE_RANGE.MIN || filters.tempRange[1] !== TEMPERATURE_RANGE.MAX,
                    filters.humidityRange[0] !== HUMIDITY_RANGE.MIN || filters.humidityRange[1] !== HUMIDITY_RANGE.MAX
          ...
          <div className="space-y-3 relative" ref={amenitiesRef}>
            ...
            >
              <span className="text-sm text-gray-700">
                {selectedTagsCount > 0 ? `${selectedTagsCount} selected` : 'Select amenities'}
                {selectedTagsCount > 0 && (
              ...
              <svg 
                className={`w-4 h-4 text-gray-500 transition-transform ${isAmenitiesOpen ? 'rotate-180' : ''}`} 
                fill="none" 
                stroke="currentColor" 
                viewBox="0 0 24 24"
              >
            ...
            {isAmenitiesOpen && (
              <div className="absolute z-50 top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-lg shadow-xl p-4 max-h-64 overflow-y-auto">
                <div className="grid grid-cols-2 gap-2">
                  {allTags.map((tag) => (
                    <label key={tag} className="flex items-center space-x-2 cursor-pointer hover:bg-gray-50 p-2 rounded transition-colors">
                      <input
                        type="checkbox"
                        value={tag}
